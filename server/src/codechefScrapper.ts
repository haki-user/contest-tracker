...
export const getCodechefContests = async () => {
  try {
    ...
    const tableRows = await page.$$('tbody.MuiTableBody-root tr.MuiTableRow-root');
    const contests = [];
    ...
    for (const row of tableRows) {
      ...
      // Extract start date
      const startDate = await columns[1].$eval('div.jss61.jss66 div._timer__container_1c9os_518 p', (el) => {
        if(el && el.textContent) el.textContent.trim();
        return 0;
      });
